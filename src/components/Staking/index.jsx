...
function Staking() {
  ...
  const handleDaiFaucet = async () => {
    ...
    await DAIContract.methods.faucet(accounts[0], BigInt(5000000000000000000)).send({ from: accounts[0] });
  };

  const handleFamFaucet = async () => {
    const FAMContract = findContract(artifact, contract, networkID, "FAM");
    await FAMContract.methods.faucet(accounts[0], BigInt(5000000000000000000)).send({ from: accounts[0] });
  };
          ...
          <Card.Description>
            <div className="staking-datas">
              <div className="staking-datas-total-stake">
                ...
                <p>{Web3.utils.fromWei(BigInt(tokenToDisplay.stakedBalance), 'ether')} {tokenToDisplay.symbol}</p>
              </div>
              ...
              </div>
              {
                ...
                //  <p>{Web3.utils.fromWei(BigInt(tokenToDisplay.estimatedFAMRewards), 'ether')}FAM</p>
                // </div>
              }
